...
namespace soomla {
    ...
    void CCSoomla::easyNDKCallBack(CCDictionary *parameters) {
		...
		} else if (methodName->compare("CCEventHandler::onHWMetadataChangedEvent") == 0) {
			CCSetIterator i;
			for(i = mEventHandlers.begin(); i != mEventHandlers.end(); i++) {
                CCEventHandler *h = dynamic_cast<CCEventHandler *>(*i);
				h->onHWMetadataChanged();
			}
		} else if (methodName->compare("CCEventHandler::onHWBalancesUpdatedEvent") == 0) {
			CCSetIterator i;
			for(i = mEventHandlers.begin(); i != mEventHandlers.end(); i++) {
                CCEventHandler *h = dynamic_cast<CCEventHandler *>(*i);
				h->onHWBalancesUpdated();
			}
		}
