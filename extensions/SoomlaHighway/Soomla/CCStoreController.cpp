...
#include "Storefront/CCStorefrontController.h"
...
namespace soomla {
    ...
    bool CCStoreController::init(CCIStoreAssets *storeAssets, CCDictionary *storeParams) {
        ...
        CCDictionary *hooksDict = dynamic_cast<CCDictionary *>(storeParams->objectForKey("hooks"));

        if (customSecret == NULL) customSecret = CCString::create("");
        if (soomSec      == NULL) soomSec      = CCString::create("");
        if (masterKey    == NULL) masterKey    = CCString::create("");
        if (genTime      == NULL) genTime      = CCDouble::create(-1);
        if (hooksDict    == NULL) hooksDict    = CCDictionary::create();
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
        ...
        CCDICT_FOREACH(storeParams, el) {
            ...
            if (!(
                  ...
                  key.compare("masterKey")    == 0 ||
                  key.compare("genTime")      == 0 ||
                  key.compare("hooks")        == 0 )) {
                CCString *message =
        ...
        CCDictionary *storefrontParams = CCDictionary::create();
        storefrontParams->setObject(masterKey, "masterKey");
        storefrontParams->setObject(genTime, "genTime");
        storefrontParams->setObject(customSecret, "customSecret");
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
        storefrontParams->setObject(androidPublicKey, "androidPublicKey");
#endif

        CCStorefrontController::sharedStorefrontController()->initHighway(storefrontParams, hooksDict);
        return true;
    }
